...
document.addEventListener("DOMContentLoaded", () => {
    ...
    writeReviewBtn.addEventListener("click", saveHikeDocumentIDAndRedirect); //add event listener to review button. when the button is clicked, run function
});

function saveHikeDocumentIDAndRedirect() {
    const params = new URL(window.location.href); //redirect to
    const hikeID = params.searchParams.get("docID"); //save docID of the hike that you want to do a review for
    ...
    localStorage.setItem("hikeDocID", hikeID); //storing hike's docID in the browser's local storage before navigating to review page
...
async function populateReviews() {
    ...
    try {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            reviewCard.querySelector(".season").textContent =
                `Season: ${season}`;
            reviewCard.querySelector(".scrambled").textContent =
                `Scrambled: ${scrambled}`;
            reviewCard.querySelector(".flooded").textContent =
                `Flooded: ${flooded}`;
